...
void v_drv_w25q_page_write_no_check(uint8_t* puc_srs, uint32_t ul_addr,
                                    uint16_t us_len) {
  prvDrvSPIFlashWriteEnable();
  prv_drv_w25q_wait_for_write_end();
...
void v_drv_w25q_brust_write_no_check(uint8_t* puc_srs, uint32_t ul_addr,
                                     uint16_t us_len) {
  uint16_t us_page_remain = W25QXX_PAGE_SIZE - ul_addr % W25QXX_PAGE_SIZE;

  if (us_len <= us_page_remain) us_page_remain = us_len;
...
void v_drv_w25q_brust_write_check(uint8_t* puc_srs, uint32_t ul_addr,
                                  uint16_t us_len) {
  ...
  while (1) {
    v_drv_w25q_read(puc_temp_buf, us_sec_position * W25QXX_SECTOR_SIZE,
                    W25QXX_SECTOR_SIZE);

    for (i = 0; i < us_sec_remain; i++) {
      if (puc_temp_buf[us_sec_offset + i] != 0xff) break;
    }
